...
readChunkLen( int sockFD )
{
	...
	int retval = -1;
	if  (sscanf(strform, "%x", (unsigned int *)&retval) <= 0 )
	{
		free(strform);
		return -1;
	}
...
parseHTTPHeaders( char * input, char ** endsHere )
{
	...
	char * hpattern = "^([^:]+)[ ]*:[ ]*([^\r]+)\r\n";
	regcomp( &pair, hpattern, REG_EXTENDED | REG_ICASE );
...
main( int argc, char* argv[] )
{
	...
	while (1)
	{
		...
		if ( f1.code/100 == 2 )	//OK
		{
			fprintf(stderr,"All OK saving file...");
			saveresfile(&f1, ran);
			fprintf(stderr,"DONE\n");
			freeResponse(&f1);
			free(toopen);
		...
		if ( f1.code/100 == 3 )	//Redirects
		{
			...
			if ( newurl == NULL )
			{
				fprintf(stderr,"Response code: %d, missing \"Location\" header field", f1.code);
				freeResponse(&f1);
				exit(1);
			}
			hlp = parseUrl(newurl);
